...
void init(){
	...
	output += "p cnf ";
	output += to_string(no_var);
	output += ' ';
	output += to_string(no_clauses);
	output += '\n';
}
	...
	for(int i=1;i<=k;i++){
		for(int j=1;j<=k;j++){
			...
			for(int p=1;p<=n;p++){
				...
				output += '-'; output += to_string(curr_var_no); output += ' '; output += to_string(pi); output += " 0\n";
				output += '-'; output += to_string(curr_var_no); output += " -"; output += to_string(pj); output += " 0\n";
				output += to_string(curr_var_no); output += " -"; output += to_string(pi); output += ' '; output += to_string(pj); output += " 0\n"; 
			}
			// fout << clause << "0\n";
			output += clause;
			output += "0\n";
...
void add_edge_clause(int i, int j){
	...
	for(int l=1;l<=k;l++){
		...
		output += '-'; output += to_string(curr_var_no); output += ' '; output += to_string(il); output += " 0\n";
		output += '-'; output += to_string(curr_var_no); output += " "; output += to_string(jl); output += " 0\n";
		output += to_string(curr_var_no); output += " -"; output += to_string(il); output += " -"; output += to_string(jl); output += " 0\n"; 
	}
	// fout << clause << "0\n";
	output += clause;
	output += "0\n";
