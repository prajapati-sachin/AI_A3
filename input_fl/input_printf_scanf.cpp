	...
	for(int i=1;i<=k;i++){
		for(int j=1;j<=k;j++){
			...
			for(int p=1;p<=n;p++){
				...
				// cout << '-' << curr_var_no << ' ' << pi << " 0\n";
				// cout << '-' << curr_var_no << ' ' << '-' << pj << " 0\n";
				// cout << curr_var_no << ' ' << '-' << pi << ' ' << pj << " 0\n";
				printf("-%d %d 0\n",curr_var_no,pi);
				printf("-%d -%d 0\n",curr_var_no,pj);
				printf("%d -%d %d 0\n",curr_var_no,pi,pj);
			}
...
void add_edge_clause(int i, int j){
	...
	for(int l=1;l<=k;l++){
		...
		// cout << '-' << curr_var_no << ' ' << il << " 0\n";
		// cout << '-' << curr_var_no << ' ' << jl << " 0\n";
		// cout <<        curr_var_no << " -" << il << " -" << jl << " 0\n";
...
int main(int argc, char const *argv[]){
	...
	// char *in1 = inputFile.c_str();
	// char *out1 = ouputFile.c_str();
