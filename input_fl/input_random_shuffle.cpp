...
void lets_do_it(){
	...
	while(!se.empty()){
		...
		string yuo;
		yuo += to_string(vertex_var_no(curr_vertex, subgraph_index));
		yuo += " 0";
		ans.push_back({yuo,1});
		...
		for(int i=1;i<=n;i++){
			...
			yuo.clear();
			yuo += to_string(-vertex_var_no(i, subgraph_index));
			yuo += " 0";
			ans.push_back({yuo,1});
...
bool compare_it(pair<int,int> a, pair<int,int> b){
	if(a.first == b.first){
		int c = rand() % 2;
		if(c == 0) return true;
		else return false;
...
int main(int argc, char const *argv[]){
	...
	vector< pair<int,int> > shuffler(ans.size());
	for(int i=0;i<shuffler.size();i++)
		shuffler[i] = {ans[i].second,i};

	sort(shuffler.begin(), shuffler.end(), compare_it);

	for(auto z : shuffler){
		// cout << z << ' ';
		fout << ans[z.second].first << '\n';
	}
	// for(auto z : ans){
	// 	fout << z << '\n';
	// }
